...
class GripperController(object):
    def __init__(self, namespace='', prefix=None,
                 timeout=5.0, attach_link='robot::wrist_3_link'):
        self.ns = utils.solve_namespace(namespace)
        self.prefix = prefix if prefix is not None else ''
        node_name = "gripper_controller"
        self.gripper_type = str(rospy.get_param(self.ns + node_name
                                                + "/gripper_type"))
        self.valid_joint_names = []
        if rospy.has_param(self.ns + node_name + "/joint"):
            self.valid_joint_names = [rospy.get_param(self.ns + node_name
                                                      + "/joint")]
        elif rospy.has_param(self.ns + node_name + "/joint"):
            self.valid_joint_names = rospy.get_param(self.ns + node_name
                                                     + "/joints")
        else:
            rospy.logerr("Couldn't find valid joints params in %s"
                         % (self.ns + node_name))
            return
        ...
        while not hasattr(self, '_joint_names'):
            if (rospy.get_time() - start_time) > timeout and not retry:
                ...
                self._js_sub = rospy.Subscriber('%sjoint_states'
                                                % self.ns, JointState,
        ...
        try:
            ...
            if attach_plugin:
                ...
                self.attach_srv = rospy.ServiceProxy(
                                        '/link_attacher_node/attach', Attach)
                self.detach_srv = rospy.ServiceProxy(
                                        '/link_attacher_node/detach', Attach)
                rospy.logdebug('Waiting for service: {0}'
                               .format(self.attach_srv.resolved_name))
                rospy.logdebug('Waiting for service: {0}'
                               .format(self.detach_srv.resolved_name))
        ...
        if not server_up:
            ...
            raise rospy.ROSException('GripperCommandAction timed out: {0}'
                                     .format(action_server))
        rospy.logdebug('Successfully connected to [%s]' % action_server)
        rospy.loginfo('GripperCommandAction initialized. ns: {0}'
                      .format(self.ns))
    ...
    def command(self, value, percentage=False, wait=True):
        ...
        if wait:
            self._client.send_goal_and_wait(self._goal,
                                            execute_timeout=rospy.Duration(2))
        else:
...
class JointControllerBase(object):
    ...
    def __init__(self, namespace, timeout, joint_names=None):
        ...
        from the same node.
        """
        self.valid_joint_names = constants.JOINT_ORDER \
        ...
        while not hasattr(self, '_joint_names'):
            if (rospy.get_time() - start_time) > timeout and not retry:
                # Re-try with namespace
                self._js_sub = rospy.Subscriber('%sjoint_states' % self.ns,
                                                JointState,
                                                self.joint_states_cb,
                                                queue_size=1)
                start_time = rospy.get_time()
...
class JointPositionController(JointControllerBase):
    ...
    def __init__(self, namespace='', timeout=5.0, joint_names=None):
        ...
        @param timeout: Time in seconds that will wait for the controller
        """
        super(JointPositionController, self).__init__(namespace,
                                                      timeout=timeout,
        ...
        for joint in self._joint_names:
            ...
            self._cmd_pub[joint] = rospy.Publisher('%s%s/command'
                                                   % (self.ns, joint),
        ...
        list_controllers = rospy.ServiceProxy(controller_list_srv,
                                              ListControllers)
        expected_controllers = (joint_names if joint_names is not None else
                                constants.JOINT_ORDER)
        start_time = rospy.get_time()
        while not rospy.is_shutdown():
            ...
            if found == len(expected_controllers):
                break
        rospy.loginfo('JointPositionController initialized. ns: {0}'
                      .format(namespace))
...
class JointTrajectoryController(JointControllerBase):
    ...
    def __init__(self, publisher_name='arm_controller', namespace='',
                 timeout=5.0, joint_names=None):
        """
        ...
        @param timeout: Time in seconds that will wait for the controller
        """
        super(JointTrajectoryController, self).__init__(namespace,
                                                        timeout=timeout,
                                                        joint_names=joint_names)
        ...
        rospy.loginfo('JointTrajectoryController initialized. ns: {0}'
                      .format(self.ns))
    ...
    def start(self, delay=0.1, wait=False):
        ...
        rospy.logdebug('Executing Joint Trajectory with {0} points'
                       .format(num_points))
...
class FTsensor(object):
    ...
    def __init__(self, namespace='', timeout=3.0):
        ...
        if not utils.wait_for(lambda: self.raw_msg is not None,
                              timeout=timeout):
    ...
    def cb_raw(self, msg):
        ...
        self.add_wrench_observation(
            conversions.from_wrench(self.raw_msg.wrench))
