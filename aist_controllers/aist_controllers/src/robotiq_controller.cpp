...
GripperOutput
goalToRegisterState(const GripperCommandGoal& goal,
		    const Robotiq2FGripperParams& params)
{
    ...
    if (goal.command.position > params.max_gap_ ||
	goal.command.position < params.min_gap_)
    {
    ...
    if (goal.command.max_effort < params.min_effort_ ||
	goal.command.max_effort > params.max_effort_)
    {
	ROS_WARN("Goal gripper effort out of range (%f to %f N): %f N",
		 params.min_effort_,
		 params.max_effort_, goal.command.max_effort);
	throw BadArgumentsError();
    }
    ...
    result.rPR = static_cast<uint8_t>(
		  (params.max_gap_ - goal.command.position)/dist_per_tick);
    result.rFR = static_cast<uint8_t>(
		  (goal.command.max_effort - params.min_effort_)/eff_per_tick);
...
/*  This function is templatized because both GripperCommandResult
    and GripperCommandFeedback consist of the same fields yet have
    different types. Templates here act as a "duck typing" mechanism
    to avoid code duplication.
*/
template<typename T> T
registerStateToResultT(const GripperInput& input,
		       const Robotiq2FGripperParams& params,
		       uint8_t goal_pos)
{
    T result;
...
void
Robotiq2FGripperActionServer::goalCB()
{
...
void
Robotiq2FGripperActionServer::preemptCB()
{
...
void
Robotiq2FGripperActionServer::analysisCB(const GripperInput::ConstPtr& msg)
{
  ...
  // Check to see if the gripper is in its activated state
    if (current_reg_state_.gSTA != 0x3)
    {
      // Check to see if the gripper is active or if it has been asked
      // to be active
	if (current_reg_state_.gSTA == 0x0 && goal_reg_state_.rACT != 0x1)
	{
      ...
      // TODO: If message delivery isn't guaranteed,
      // then we may want to resend activate
	return;
    }
  ...
  // Check for errors
    ...
    {
	ROS_WARN("%s faulted with code: %x",
		 action_name_.c_str(), current_reg_state_.gFLT);
	as_.setAborted(registerStateToResult(current_reg_state_,
					     gripper_params_,
					     goal_reg_state_.rPR));
    }
    else if (current_reg_state_.gGTO &&
	     current_reg_state_.gOBJ &&
	     current_reg_state_.gPR == goal_reg_state_.rPR)
    {
      // If commanded to move and if at a goal state and if the position
      // request matches the echo'd PR, we're done with a move
...
void
Robotiq2FGripperActionServer::issueActivation()
{
