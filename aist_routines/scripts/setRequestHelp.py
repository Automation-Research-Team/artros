...
def callback_RequestHelp(data):
    ...
    msg_pose = {"position":{"x":-pose.position.y,"y":pose.position.z,"z":pose.position.x},
            "orientation":{"x":-pose.orientation.x,"y":pose.orientation.z,"z":pose.orientation.x,"w":pose.orientation.w }}
    request = data.request
    message = data.message
    pubRequestHelp.publish({"robot_name":robot_name, "item_id": item_id, "pose":msg_pose, 'request':request, 'message':message})
